function [V_final,F_final,T_final,BF,Ncells,Acell,Amod,ML,MW] = moduleGeometryTracking_hsats(TOOLBOX_input,CELL_output,BackwardTracer,Submod_ind)
% moduleGeometryTracking_hsats builds 3D module geometry of module and frame in a horizontal single-axis tracking system.
%
...
V_cell = moveit(V_cell, [0,0,pole_W/2]);
...
for i = 1:N_modules
    ...
    %Start_index = 2*Ncells*(1+BF)+2*(i-1);
...
%optical prop of modules (if bifacial less light will be reflected)
...
V_struc = moveit(V_struc, [0,0,pole_W/2]);
...
x_coor_poles = [-DS*Nmod_side-pole_W,DS*Nmod_side+MW];
...
[V,F] = rectabox(x_coor_poles(2),y_coor_poles,0,pole_W,pole_W,HG-pole_W);
[V_struc,F_struc] = combine({V_struc,V},{F_struc,F},[],BackwardTracer);
...
[V,F] = rectabox(0,-pole_W/2,-pole_W/2,DS*Nmod_side*2+MW,pole_W,pole_W);
...
if NC~=0
    x_struc_col = 1:1:NC;
    x_struc_col = (ICD+DS*Nmod_side*2+MW).*[-x_struc_col,x_struc_col];
...
[V,F] = rectabox(-10000,-10000,0,20000,20000,0); %Make ground plane
T.Facet = [1,2];
