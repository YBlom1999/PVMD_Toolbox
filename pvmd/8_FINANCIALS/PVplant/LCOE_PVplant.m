function [LCOE_output, TOOLBOX_input] = LCOE_PVplant(TOOLBOX_input,ELECTRIC_output,DEGRADATION_output)
% LCOE_farm calculates the costs of the PV system operating in the field.
%
% This function calculates the LCOE in 3 scenarios
% 1-When the same module is used for expected lifetime without any
% replacement.
% 2-When the modules are replaced by the same module (without any
% technological changes) after the lifetime is reached.
% 3-When the modules are replaced by new technology after reaching its
% lifetime.
%
% Parameters
% ----------
...
nummod = TOOLBOX_input.Conversion.Parallel_Modules*...
    TOOLBOX_input.Conversion.Series_Modules;
...
if ~TOOLBOX_input.script
    ...
    TOOLBOX_input.FinancialPart.PlantScenario = result;
else
    result = TOOLBOX_input.FinancialPart.PlantScenario;
end
...
elseif result==2
    ...
    LCOE_output.S3.Energy_3=Energy_3;
    
    %Plot LCOE vs years
    if ~TOOLBOX_input.script
        LCOE_plots(result,Cost_1,Cost_2,Cost_3);
    end
end
