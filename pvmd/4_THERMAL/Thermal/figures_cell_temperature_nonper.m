...
%
% Developed by K Ganapathi Subramanian, Adjusted by Y. Blom

mod_arr = TOOLBOX_input.Scene.N_panels;
[m,n] = size(mod_arr);
x = 1:length(ambient_temp);

for i = 1:numel(mod_arr)
    temp_to_plot = cell_temp{i};
    temp_to_plot = mean(temp_to_plot,2);
    irr_to_plot = irradiance{i};
    irr_to_plot = mean(irr_to_plot,2);
    figure(36);
    subplot(m,n,i); 
