function [pvt_collector_output] = HP_new(~,~, WEATHER_output)
...
Tc = zeros(size(Tam));
Two = zeros(size(Tam));
total_thermal_output = 0;
total_electrical_output = 0;

% These can be changed 
T_des = 50;                                     % Desired temperature of room
...
for i=1:n
    T1(1,i) = Tam(i) - T_evap_appr;                                                    % Evaporator temperature in °C
    ...
    E_input = E_input + (Q_load/(365*24))*P_hp(1,i)/Q_amb(1,i);                        % Cummulatively adding the energy input
