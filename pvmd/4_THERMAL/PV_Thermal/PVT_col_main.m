function [pvt_collector_output] = PVT_col_main(TOOLBOX_input, MODULE_output, WEATHER_output)
...
[pvt_collector_output] = fh(TOOLBOX_input,MODULE_output, WEATHER_output);
...
if ~TOOLBOX_input.script
    if pvt_collector_output.total_electrical_output>0 && pvt_collector_output.total_thermal_output>0
    ...
    elseif pvt_collector_output.total_electrical_output>0 && pvt_collector_output.total_thermal_output==0
        disp('PV Calculation finnished, PV yield calculated.');
        disp(['Total Electrical Yield in given period:', num2str(pvt_collector_output.total_electrical_output), 'kWh/m^2']);
