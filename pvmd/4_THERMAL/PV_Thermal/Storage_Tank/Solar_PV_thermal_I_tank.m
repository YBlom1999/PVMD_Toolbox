...
C_f = 4186;                      % specific heat of water (J/kg°C) 
...
T0 = 10;                         % Initial Temperature of the fluid entering the collector (°C)
Tt0 = 5;                         % Initial Temperature of the fluid inside tank (°C) - if put 35 takes few mins
...
L = h;                           % characterstic length (m)
d_insulation = 0.04;             % insulation thickness (m)
...
C = 0.52;                        % constant
n = 0.25;                        % constant (1/4)
L_c = 3;                         % characterstic lenght (l of hex)

Twin = 10*ones(size(Tam));
% preallocate arrays
TT = zeros(length(Tam), 1);
Tt = zeros(size(Tam));            % storage tank temperature (°C)
T_i = zeros(size(Tam));           % inlet temperature (°C)
...
Re(i) = rho_air*Vw(i)*L/mu_air;        % Reynold number
Nu(i) = 0.664*sqrt(Re(i))*Pr^(1/3);    % Nusselt number
h_wind(i) = Nu(i)*k_air/L;             % convective heat coefficient due to wind
...
eta_tha(i) = etao_tha - a1a * tmp(i) - a2a * tmp(i).^2;
% avoid negative values
eta_tha(i) = max(eta_tha(i), 0);
% avoid values greater than one
eta_tha(i) = min(eta_tha(i), 1);
...
Two(i) = T_i(i) + ((eta_tha(i)*A_g*I(i)))/(dm_f*C_f);
% cell temperature
NOCT = 55;                       % typical module at 48°C (best module operated at a NOCT of 33°C, the worst at 58°C)
S = I*0.1;                       % insolation in mW/cm^2
Tc = Tam + S*(NOCT - 20)/80;                                                    
R_a(i) = (g*beta*(Two(i) - Tt(i))*d_o^3)/(v*alpha_d); % Rayleigh number
...
NTU(i) = U_A(i)/(dm_f*C_f);                         % number of transfer units
eta(i) = 1 - exp(-NTU(i));

% Instantaneous power at normal incidence
Qa(i) = eta_tha(i)*A_g*I(i);
% % Instantaneous electrical power at normal incidence
% Qe = eta_E .* A_g .* G;
if i < length(Tam) 
% Collector to tank
qctota(i) = eta(i)*dm_f*C_f*(Two(i) - Tt(i));
% Tank loss
qloss(i) = U_t(i)*A_t*(Tt(i) - Tam(i));
% DHW load
qload(i) = dm_l(i)*C_f*(Tt(i) - Twin(i)); % for real load put 60*C instead of Tt (chap 6: space heating and cooling)
UA_l = 2500;                                        % space loss coefficient & area product - W/*C 
T_r = 21;                                           % desired air room temperature
eta_l = 0.7;                                        % heat exchange effectiveness
dm_a = 1.1;                                         % flow rate
C_a = 1007;                                         % specific heat of air (J/kg°C) 
% Space heating load - (desired room temperature = 21*C)
...
q_lcmax(i) = eta_l*dm_a*C_a*(Tt(i) - T_r);
...
qaux(i) = dm_l(i)*C_f*(50 - Tt(i));
if qaux(i)<0
    qaux(i) = 0;
end
% Total auxilary energy required
Q_aux(i) = q_hl(i) + qaux(i) - qloss(i) - q_ls(i);
delta_T = 3600;
Tt(i+1) = Tt(i) + delta_T*(qctota(i) - qloss(i) - qload(i))/(mt*C_f);          % heat loss from tank to environement: (U_t*A_t*th*(Tt(i) - Ta(i)))
% Tt(i+1) = Tt(i) + delta_T*(qctota(i))/(mt*C_f);
end
...
yyaxis right
plot(xx,qaux, 'DisplayName','Q_{aux}')
