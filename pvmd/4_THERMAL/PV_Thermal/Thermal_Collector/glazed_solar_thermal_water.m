function [pvt_collector_output] = glazed_solar_thermal_water(~,MODULE_output, WEATHER_output)
% Calculates the thermal and PV yield of water-based PVT collector
% 03-24 - Water-based glazed-ST: ths
%
% Parameters
% ----------
...
eta_tha(eta_tha<0) = 0;
eta_tha(eta_tha>0.80) = 0.80;
eta_tha(Ia<=1) = 0;

Quth = Ia.*eta_tha;

pvt_collector_output.y(:, 1) = Tg;        % Store Tg values in the first column of y
pvt_collector_output.y(:, 2) = Tcf;       % Store Tcf values in the second column of y
pvt_collector_output.y(:, 3) = Tr;        % Store Tt values in the third column of y
pvt_collector_output.y(:, 4) = Tm;        % Store Tt values in the fourth column of y
pvt_collector_output.y(:, 5) = Tw;        % Store Two values in the fifth column of y
pvt_collector_output.y(:, 6) = Ti;        % Store Two values in the sixth column of y
pvt_collector_output.y(:, 7) = Two;       % Store Two values in the seventh column of y
pvt_collector_output.y(:, 8) = eta_tha;   % Store eta_tha values in the eight column of y
...
alpha_r = 0.95;                  % Absorptivity of absorber (% https://www.sciencedirect.com/science/article/pii/S2590174522000708?via%3Dihub  )
sigma = 5.67e-8;
l_gla = 0.0032;     
em_gla = 0.88;          
em_r = 0.05;                     % https://www.sciencedirect.com/science/article/pii/S2590174522000708?via%3Dihub            
...
alpha_d = lamda_f/(rou*C_f);     % thermal diffusivity - m²/s
Diff_1 = x(1) - x(2);
Diff_1(Diff_1<=0) = 0.001;
R_a_g = (g*beta_t*(delta_a^3)*(Diff_1))/(mu_f*alpha_d);     % Diff_1 = x(1) - x(2)
Diff_2 = x(3) - x(2);
Diff_2(Diff_2<=0) = 0.001;
R_a_r = (g*beta_t*(delta_a^3)*(Diff_2))/(mu_f*alpha_d);     % Diff_2 = x(3) - x(2)

A_gone = (1 - (1708)/(R_a_g*cos(pi/6)));
A_gone = max(A_gone, 0);       % Take only positive values by setting negative values to zero
A_gtwo = (((R_a_g*cos((pi/6))/5830)^(1/3)) - 1);
A_gtwo = max(A_gtwo, 0);       % Take only positive values by setting negative values to zero
Nua_g = (1 + 1.44*(A_gone*(1 - (1708*(sin(1.8*(pi/6)))^1.6)/(R_a_g*cos((pi/6))))) + A_gtwo);
B_rone = (1 - (1708)/(R_a_r*cos(pi/6)));
B_rone = max(B_rone, 0);       % Take only positive values by setting negative values to zero
B_rtwo = (((R_a_r*cos((pi/6))/5830)^(1/3)) - 1);
B_rtwo = max(B_rtwo, 0);       % Take only positive values by setting negative values to zero
Nua_r = (1 + 1.44*(B_rone*(1 - (1708*(sin(1.8*(pi/6)))^1.6)/(R_a_r*cos((pi/6))))) + B_rtwo);

hvgmcf = (Nua_g*lamda_f)/delta_a;
hvcfmr = (Nua_r*lamda_f)/delta_a;
hrgmr = sigma*((x(1) + x(3))*(x(1)^2 + x(3)^2))/((1/em_gla)+(1/em_r) - 1);
...
x1 = (alpha_gla*Is - hrgms*(x(1) - Tsky) - hvgmcf*(x(1) - x(2)) - hrgmr*(x(1) - x(3)) - hva*(x(1) - T_amb))*(A_mod/(M_gla*C_gla));

%% for closed air space : x(2)
x2 = (hvcfmr*(x(3) - x(2)) - hvgmcf*(x(2) - x(1)))*(A_mod/(M_f*C_f));

%% for absorber : x(3)
x3 = (A_mod*(tau_gla*alpha_r*Is - hrgmr*(x(3) - x(1)) - hvcfmr*(x(3) - x(2))) - A_rm*hcrmm*(x(3) - x(4)) - A_ri*hcrmi*(x(3) - x(6)))*(1/(M_r*C_r));

%% for tube : x(4)
x4 = (A_rm*hcrmm*(x(3) - x(4)) - A_mi*hcmmi*(x(4) - x(6)) - A_mw_hvmmw*(x(4) - x(5)))*(1/(M_m*C_m));

%% for outlet water : T_wo
Two = 2*x(5) - Twin;

%% for water : x(5)
x5 = (A_mw_hvmmw*(x(4) - x(5)) + dm_w*C_w*(Twin - Two))*(1/(M_w*C_w));

%% for insulator : x(6)
x6 = (A_mi*hcmmi*(x(4) - x(6)) + A_ri*hcrmi*(x(3) - x(6)) - A_mod*hva*(x(6) - T_amb))*(1/(M_i*C_ins));
