function [V_module,reconfig_setting] = connectCells2ModuleReconfig(Vcell,numCells,I,day,night,Rsw,algo,nc_r)
%%Combines all cell VI curves into one module IV curve for reconfigurable
%%modules
%Input:
%Vcell -Cell VI curves
...
V_block = zeros((size(Vcell,1)/numCells)*blocks , length(I));
for h=1:size(day)
    V_cells = Vcell((h-1)*numCells+1:h*numCells , :);
    for b=1:blocks
        bx = (h-1)*blocks+b;
