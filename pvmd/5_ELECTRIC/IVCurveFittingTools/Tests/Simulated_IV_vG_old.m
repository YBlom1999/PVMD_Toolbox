function [V_,day,night] = Simulated_IV_vG_old(Acell,J_,T_,Op,numCells,I,IVtype)
...
for i=1:length(cond(:,1))
        ...
        %Increased absorption with temperature
        Iph=Acell*J.*(1+3e-4.*(T-298.15));
        Vth=K*T/q;
        n=polyval(n_T,T).*polyval(n_G,J)./polyval(n_T,298.15);
        %due to fitting JV from asa  Rs & Rsh would be m2*Ohm and I0 would be in A/m2 without corrrection, 
        Rsh=(1/Acell)*exp(polyval(R_sh_T,T)).*exp(polyval(R_sh_G,J))./exp(polyval(R_sh_T,298.15));
        Rs=(1/Acell)*exp(polyval(R_s_T,T)).*exp(polyval(R_s_G,J))./exp(polyval(R_s_T,298.15));
        I0=Acell*exp(polyval(I0_T,T)).*exp(polyval(I0_G,J))./exp(polyval(I0_T,298.15)) ;
...
%exporting diode parameters for testing purposes
save diodeParamerter.mat export_param;
